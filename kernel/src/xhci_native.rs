...
//! Phase 2 MVP: command/event rings, root-port reset, EP0 enumeration, interrupt-IN polling.
...
#[inline]
unsafe fn op_read64(off: u32) -> u64 {
    let lo = op_read32(off) as u64;
    let hi = op_read32(off + 4) as u64;
    lo | (hi << 32)
}
...
unsafe fn address_device(slot: u8, port: u8) -> Result<(), ()> {
    let dev_phys = phys(core::ptr::addr_of!(DEV_CTX) as usize);
    DCBAAP[slot as usize] = dev_phys;
    build_ep0_context(&mut DEV_CTX, 8);

    INPUT_CTX.fill(0);
    // Input control context: add slot + EP0 (A0 + A1).
    INPUT_CTX[0] = 0x03;
    // Slot context copy from device template (speed from port).
    INPUT_CTX[0x20..0x40].copy_from_slice(&DEV_CTX[0..0x20]);
    INPUT_CTX[0x40..0x60].copy_from_slice(&DEV_CTX[0x20..0x40]);
...
unsafe fn fetch_config(slot: u8, addr: u8) -> Result<usize, ()> {
...
unsafe fn set_configuration(slot: u8, addr: u8, cfg: u8) -> Result<(), ()> {
...
unsafe fn configure_interrupt_ep(_slot: u8, ep: u8, mps: u16) -> Result<(), ()> {
    let ep_idx = (ep & 0x0F) * 2 + 1;
    let off = 0x20 + (ep_idx as usize) * 0x20;
    if off + 0x20 > DEV_CTX.len() {
        return Err(());
    }
    let ep_ctx = &mut DEV_CTX[off..off + 0x20];
    ep_ctx.fill(0);
    ep_ctx[1] = 3; // interrupt
    ep_ctx[4] = ep;
    ep_ctx[6] = (mps & 0xFF) as u8;
    ep_ctx[7] = (mps >> 8) as u8;
    ep_ctx[11] = 8;
    set_ep_ring(&mut DEV_CTX, off, core::ptr::addr_of!(KBD_RING) as usize);
    KBD_ENQ = 0;
    KBD_CYCLE = true;
    ...
    INPUT_CTX[0x20..0x40].copy_from_slice(&DEV_CTX[0..0x20]);
    INPUT_CTX[off..off + 0x20].copy_from_slice(ep_ctx);
...
unsafe fn poll_kbd_interrupt(slot: u8) -> Result<usize, ()> {
    let mps = KBD_MPS;
    let ep_doorbell = u32::from(KBD_EP & 0x0F);
    let ring = core::ptr::addr_of_mut!(KBD_RING).cast::<Trb>();
    let buf_phys = phys(core::ptr::addr_of!(IRQ_KBD) as usize);
    push_transfer(
        ring,
        &mut KBD_ENQ,
        &mut KBD_CYCLE,
...
unsafe fn try_enumerate_keyboard(slot: u8, addr: u8) -> Result<(), ()> {
    let n = fetch_config(slot, addr)?;
    if n < 9 {
        return Err(());
    }
    let cfg = DATA[5];
    set_configuration(slot, addr, cfg)?;
    let Some((_, ep, mps)) = find_hid_boot_eps(&DATA[..n], 1) else {
        return Err(());
    };
    configure_interrupt_ep(slot, ep, mps)?;
    KBD_EP = ep;
    KBD_MPS = mps.max(8);
    KBD_READY = true;
    Ok(())
...
/// Initialize native xHCI and enumerate a boot keyboard on the first connected root port.
pub unsafe fn init(skew: u64, mmio: usize) -> bool {
    ...
    for port in 0..MAX_PORTS {
        ...
        if address_device(slot, port).is_err() {
            continue;
        }
        if set_address(slot, 1).is_err() {
            continue;
        }
        if try_enumerate_keyboard(slot, 1).is_ok() {
            SLOT = slot;
...
pub unsafe fn poll_keyboard_report() -> Option<[u8; 8]> {
    ...
    IRQ_KBD.fill(0);
    if poll_kbd_interrupt(SLOT).is_err() {
