...
In Eve **Settings (SYS)**: leave **NIC** on **VirtIO** (or **E1000** if the VM uses `-device e1000`), turn **INTERNET** (or equivalent stack toggle) **on**,
then in the browser bar enter a URL and press Enter. **`https://www.google.com/`** is the default home;
**HTTP** works the same if the site allows it.
